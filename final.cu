...
int main(int argc, const char *argv[]){
    ...
    CHECK(cudaMemcpy(dev_semaphores, host_semaphores, num_rows * sizeof(char), cudaMemcpyHostToDevice));
    ...
    // divide forward and backward sweep into chunks, rerun each chunk until all values have been found
    // forward sweep
    ...
    // check for errors, sensibility is needed because gpu values slightly differ
    int errors = 0;
    float maxError = 0.0;
    for(int i = 0; i < num_rows; i++){
